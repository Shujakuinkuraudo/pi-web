...
test("renders the SDK compaction-aware context with aligned entry IDs", () => {
  ...
  assert.deepEqual(context.entryTimestamps, [
...
test("uses only the latest compaction on the active path", () => {
  ...
  assert.equal(context.entryTimestamps.length, context.entryIds.length);
});
...
test("uses null for an invalid persisted timestamp without breaking alignment", () => {
  const entries = [
    userEntry("u1", null, "start", "2026-01-01T00:00:01.000Z"),
    assistantEntry("a1", "u1", "done", "invalid"),
  ];
...
test("rejects symlinked project directories during targeted resolution", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-targeted-project-link-"));
  const externalDir = mkdtempSync(join(tmpdir(), "pi-web-targeted-external-project-"));
  const sessionId = "linked-project-session";
  const externalPath = join(externalDir, `2026-01-01T00-00-00-000Z_${sessionId}.jsonl`);
...
test("rejects symlinked session files during targeted resolution", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-targeted-file-link-"));
  const externalDir = mkdtempSync(join(tmpdir(), "pi-web-targeted-external-file-"));
  const sessionId = "linked-file-session";
  const projectDir = join(agentDir, "sessions", "project");
  const externalPath = join(externalDir, "outside.jsonl");
  const linkedPath = join(projectDir, `2026-01-01T00-00-00-000Z_${sessionId}.jsonl`);
  mkdirSync(projectDir, { recursive: true });
  writeFileSync(externalPath, `${JSON.stringify({
    type: "session",
    version: 3,
  ...
  symlinkSync(externalPath, linkedPath, "file");
...
test("falls back to the catalogue when a targeted lookup finds duplicate IDs", async (t) => {
  ...
  const originalListAll = SessionManager.listAll;
  const fallbackPath = join(agentDir, "sessions", "catalogue", "fallback.jsonl");
  let scans = 0;
  SessionManager.listAll = async () => {
    scans += 1;
    return [{
...
test("reports aggregate cold and warm list timings without session metadata", async (t) => {
  const originalListAll = SessionManager.listAll;
  SessionManager.listAll = async () => [];
  resetSessionListState();
  t.after(() => {
  ...
  const cold = await listAllSessionsWithTimings({ force: true });
  const warm = await listAllSessionsWithTimings();

  assert.deepEqual(cold.sessions, []);
  assert.equal(cold.timings.cache, "miss");
  assert.ok(Number.isFinite(cold.timings.catalogueMs));
  assert.ok(Number.isFinite(cold.timings.projectsMs));
  assert.equal(warm.timings.cache, "hit");
  assert.equal(warm.timings.catalogueMs, 0);
  assert.equal(warm.timings.projectsMs, 0);
});
