...
test("derives persisted turn timing only from valid ordered timestamps", () => {
  assert.deepEqual(getTurnTiming(1_000, 66_000), {
    startedAt: 1_000,
    endedAt: 66_000,
    durationMs: 65_000,
  });
  assert.equal(getTurnTiming(null, 66_000), undefined);
  assert.equal(getTurnTiming(1_000, undefined), undefined);
  assert.equal(getTurnTiming(Number.NaN, 66_000), undefined);
  assert.equal(getTurnTiming(66_000, 1_000), undefined);
});

test("uses the last persisted entry for a completed turn and hides live tails", () => {
  const timestamps = [1_000, 4_000, 13_000];
  assert.deepEqual(getCompletedTurnTiming(timestamps, 0, 3), {
    startedAt: 1_000,
    endedAt: 13_000,
    durationMs: 12_000,
  });
  assert.equal(getCompletedTurnTiming(timestamps, 0, 3, true), undefined);
  assert.equal(getCompletedTurnTiming(timestamps, 0, 1), undefined);
});

test("formats compact turn durations without overstating sub-second work", () => {
  ...
  assert.equal(formatTurnDuration(3_665_000), "1h 1m 5s");
  assert.equal(formatTurnDuration(-1), "");
});
